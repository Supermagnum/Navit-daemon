...
class FixedIMUSource(IMUSource):
    ...
    def __init__(self, accel: tuple, gyro: tuple) -> None:
        self._accel = accel
        self._gyro = gyro
...
def test_calibrated_applies_bias_and_offset() -> None:
    inner = FixedIMUSource((0.0, 0.0, 9.81), (1.0, 2.0, 3.0))
    cal = Calibration(gyro_bias=(0.5, 0.5, 0.5), accel_offset=(0.0, 0.0, 0.1))
...
def test_calibrated_with_manager_feeds_gyro_on_read() -> None:
    ...
    inner = FixedIMUSource((0.0, 0.0, 9.81), (0.1, 0.2, 0.3))
    cal = Calibration()
